...
async function main() {
    const walletPassPhrase = askQuestion('Enter your wallet passphrase: ', true);
    const amount = askQuestion('Enter the uPokt amount to send each receiver. [Note this is not POKT amount]: ');
    const rpcProviderUrl = askQuestion('Enter your POKT RPC Provider URL: ');
    ...
    const outputFileName = `${new Date().toISOString().replace(/:/g,"_")}-results.csv`
...
function askQuestion(question: string, isPassword: boolean = false) {
    return readlineSync.question(question, {
        hideEchoBack: isPassword // This hides the user's input
    })
}
